...
var JotForm = {
    ...
    init: function (callback) {
        ...
        var ready = function () {
            try {
                ...
                // event id generates from very top of the initializion to avoid being interrupted by a code break
                if (!isPreview) {
        ...
        const { getLoops} = this.conditionDebugger();
    ...
    */
    ...
    handleScreenshot: function () {
        var $this = this;
        setTimeout(function () {
            document.querySelectorAll('.form-screen-button').forEach(function (button) {
                ...
                if (window.parent && window.parent.JotformFeedbackManager) {
                    $this.getContainer(button).style.display = '';
        ...
        if ((JotForm.newPaymentUI) && window.paymentType === 'product' && window.FORM_MODE !== 'cardform') {
            ...
            Array.from(productItems).forEach(function(productItem) {
                productItem.addEventListener('click', function(event) {
                    ...
                    if(JotForm.categoryConnectedProducts){
                        const connectedProductItemsIds = JotForm.categoryConnectedProducts[this.getAttribute('pid')];
                        const categories =  document.querySelectorAll('.form-product-category-item');
                        const connectedCategoriesString = this.getAttribute('categories');
                        const filteredCategories = [];

                        //* update connected products form ınput checked
                        if(connectedProductItemsIds && connectedProductItemsIds.length > 0){
                            connectedProductItemsIds.forEach((pid) => {
                                const connectedProductItem = document.querySelector('.form-product-item[pid="' + pid + '"]');
                                const formInput = connectedProductItem.querySelector('.form-product-input');
                                if(formInput && event.target && event.target.checked !== undefined && event.target.checked !== null){
                                    formInput.checked = event.target.checked;
                                }
                            })
                        }
        ...
        $$('[name$=\[month\]]').each(function (monthElement) {
            ...
            monthElement.dateTimeCheck = function (e) {
                ...
                if (!erroredElement) {
                    ...
                    var dateInput = dateElement.down('input[class*="validate"][class*="limitDate"]');

                    if (dateInput && dateInput.hasClassName('invalidDate')) {
                        erroredElement = dateInput;
                        hasErrorInLiteModeEl = true;
                    }
                    if (dateInput && dateInput.hasClassName('invalidLimits')) {
                        erroredElement = dateInput;
                        limitError = true;
                    }
                }
        ...
        $$('.form-submit-reset').each(function (b) {
            b.onclick = function () {
                if (!confirm(JotForm.texts.confirmClearForm)) {
                    ...
                    // reset subtotals, too
                    if (JotForm.payment) {
                        if($$('span[id*="_item_subtotal"]').length > 0) {
                            var zeroValue = '0';
                        ...
                        $$('.form-product-input').each(function(item){
  ...
  */
  handleSignSignatureInputs: function() {
    ...
    corrected: function (input) {
        ...
        if(!JotForm.isValidMinTotalOrderAmount() && container.classList.contains("form-minTotalOrderAmount-error")) {
          return;
        }
        ...
        try {
            ...
            setTimeout(() => {
                ...
                Inputmask({
                    ...
                    mask: maskValue,
                    inputEventOnly: true,
                    postValidation: function(characters,t,a,isValid,n,s,l) {
