...
if ($PagoADevolver < 0){
    ...
    $servername = "localhost";
                    ...
                    $insertar = "INSERT INTO cuentasxcobrar (
                        Cedula, NombrePacientee, FechaDeConsullta, FechaDeExpiracion, TotalApagar, MetodoDePago, PagoDePaciente, RestanteAPagar
                    ) VALUES (
                        '$cedula', '$nombre','$fechaConsulta', '$fechaExpiracion', '$TotalAPagar', '$metodoPago', '$PagoRealizado', '$PagoADevolver'
                    )";
...
foreach ($_POST as $clave => $valor) {
    ...
    if (preg_match('/<td>.*?<\/td>/', $valor)) {
        ...
        if ($matches && count($matches[1]) >= 6) {
            ...
            $consulta = "UPDATE inventario SET CantidadEnStock = CantidadEnStock - $cantidad WHERE ProductoOServicio = '$productoServicio' AND Descripcion = '$descripcion' AND Categoria = '$categoria'";              //Total es DescuentoUnitario

            $insertar = "INSERT INTO factura VALUES ('','','$nombre','$cedula','$edad','','$dni','','$metodoPago', '$fechaConsulta', '$fechaExpiracion', '', '$productoServicio','$descripcion', '$categoria','$cantidad', '$precio','$total', '$TotalAPagar','$PagoRealizado','$PagoADevolver','$totalDescuento','')";
            
            $query= mysqli_query($conn, $insertar);
            // Venta a Sigui Magallanes (CI: 4024100000000) por 2 Productos, pago en efectivo, tarjeta, o depósito. Total: $4504, pagado exacto, cambio: $50. Total comprado: $4504.
            $accion = "Venta a $nombre (Cedula.: $cedula ) por $cantidad $productoServicio, pagó en.: $metodoPago, Total: $TotalAPagar, total a devolver.: $PagoADevolver"; //$apellidos
    ...
    @import url('https://fonts.googleapis.com/css2?family=Ubuntu:wght@300;400;500;700&display=swap');
        ...
        td {
            /* border-radius: 5px; */
                ...
                /* Clase para ocultar contenido específico */
        .no-imprimir {
            display: none;
        }
</style>
...
<body>
    ...
    <div class="factura">
        ...
        <div class="cliente">
            ...
            <p><strong>Edad:</strong> <?php echo $edad; ?> <span>años</span></p>
            <p><strong>Fecha:</strong> <?php echo $fechaConsulta; ?></p>
...
if (!empty($filasUnicas)) {
    // Mostrar la tabla
    echo '<div style="margin-left: -25px; margin-right: 0; margin-top: 25px; font-size:12px ;">';
    echo "<table style= max-width: 25%; width:25%;>";
